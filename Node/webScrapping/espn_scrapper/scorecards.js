...
function getMatchDetails(html) {
	...
	console.log('number of batmen tables are -> ', allBatsmenTable.length);
	let htmlString = '';
	let count = 0;
	for (let i = 0; i < allBatsmenTable.length; i++) {
		...
		for (let i = 0; i < allRows.length; i++) {
			...
			if (selecTool(firstColmnOfRow).hasClass('batsman-cell')) {
				...
				console.log(
					`playerName -> ${playerName} runsScored -> ${runs} ballsPlayed -> ${balls} numberOfFours -> ${numberOf4} numberOfSixes -> ${numberOf6} strikeRate -> ${sr}`
				);
